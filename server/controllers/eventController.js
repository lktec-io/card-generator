...
async function createEvent(req, res) {
  const {
    event_name, event_type = 'Wedding', event_date, venue,
    dress_code_main, dress_code_secondary, dress_code_notes, maps_link,
  } = req.body;
  ...
  try {
    const [result] = await pool.execute(
      `INSERT INTO events
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        event_name.trim(), event_type, event_date || null, venue?.trim() || null,
        dress_code_main?.trim() || null, dress_code_secondary?.trim() || null,
        dress_code_notes?.trim() || null, maps_link?.trim() || null,
      ]
    );
  ...
  try {
    await pool.execute(
      ...
      [
        event_name.trim(), event_type, event_date || null, venue?.trim() || null,
        dress_code_main?.trim() || null, dress_code_secondary?.trim() || null,
        dress_code_notes?.trim() || null, maps_link?.trim() || null, id,
      ]
    );
    const [[event]] = await pool.execute('SELECT * FROM events WHERE id = ?', [id]);
